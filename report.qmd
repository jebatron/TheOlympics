...
df <-df %>%
        select(-1,-5) %>% 
        filter(!is.na(Year)) %>% 
        separate(`Best Nation`, into = c("Best_Nation","num_metals"), sep = "\\(")

df <- df %>% 
       mutate(num_metals = parse_number(num_metals))
