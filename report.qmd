...
Chart of total number of goal medals won since 1896 sorted by countries rank by their GDP in 2022. The hope of this graph is to shed some light into how countries with high GDP perform in terms of gold medal count.
...
df_GDP <- read_csv(here('data',"GDP.csv"))
...
df_medals <- read_csv(here('data',"olympics_medals_country_wise.csv"))

view(df_medals)

mega_df <- left_join(df_medals, df_GDP_long, by = "countries") %>%             filter(!is.na(GDP)) %>% 
            select(-c('ioc_code','Country Code'))
...
mega_df %>% 
    mutate(
        countries = fct_reorder(countries, GDP)
    ) %>%  
    arrange(desc(GDP)) %>% 
    slice(1:10) %>%  
    ggplot(aes(x = total_gold,
               y = countries))+
    geom_col(
        fill = 'gold',
        alpha = 0.8)+
    geom_text(aes(label = total_gold),
              hjust = -0.2)+
    scale_x_continuous(expand = expansion(mult = c(0, 0.15))
    )+
    theme_classic()+
    labs(
        x = "Total Number of Gold Medals Won",
        y = "Country Ranked by GDP in 2022",
        title = "Total Number of Gold Medals Won by Coutries"
    )
...
As you can see for the most part countries with higher GDP's generally have higher gold medal counts. There are some exception as India and Brazil but, for the most part the trends is upward as the GDP increases.
